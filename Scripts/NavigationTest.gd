...
func _physics_process(delta):
	...
	if targets.size() <= 0:
		return
	var smallest_distance = (targets[0].translation - self.translation).length_squared()
	var target_entity = targets[0]
	for t in targets:
		var dist = (t.translation - self.translation).length_squared()
	...
	if attack_timer > 0:
		attack_timer -= delta
		if attack_timer <= 0:
			var diff_to_target: Vector3 = target_entity.translation - self.translation
	...
	if !attacking:
		...
		navigation.set_target_location(target_entity.translation)
